...
function Dashboard() {
  ...
  const key = useMemo(
    () => `${clientName}__${monthKey(selectedMonth)}__${personaKey(selectedPersona)}`,
    [clientName, selectedMonth, selectedPersona]
  );
  ...
  return (
    <div style={{ flex: 2, backgroundColor: "#0B111D", padding: "2rem", borderRadius: "1rem" }}>
      ...
      {/* Debug (optional): shows the lookup key and counts — remove later
      <pre style={{marginTop:"1rem", fontSize:12, opacity:0.6}}>
